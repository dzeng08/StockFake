...
struct UndoInfo {
	...
	constexpr UndoInfo() : entry(0), captured(NO_PIECE), epsq(NO_SQUARE), moveRule(0) {}

	//This preserves the entry bitboard across moves
	UndoInfo(const UndoInfo& prev) :
		entry(prev.entry), captured(NO_PIECE), epsq(NO_SQUARE), moveRule(0) {}
};
...
public:
	...
	inline bool positionRepeated() { return repetitionTable.positionRepeated(hash); }
	inline int getMoveRule() { return moveRule; }
	inline void resetBoard() {
		...
		repetitionTable.clearRepetitionTable();
		moveRule = 0;
	...
	inline bool inCheck(Color C) const {
		if (C == WHITE) return in_check<WHITE>();
		else return in_check<BLACK>();
	}
	...
	void undo(Move m) {
		if (side_to_play == BLACK) undo<WHITE>(m);
		else undo<BLACK>(m);
	}
...
void Position::play(const Move m) {
	...
	history[game_ply].moveRule = moveRule;
	moveRule++;

	if (m == Move()) {
		repetitionTable.addPosition(hash);
		return;
	}
	...
	case QUIET:
		...
		if (board[m.to()] == WHITE_PAWN || board[m.to()] == BLACK_PAWN) moveRule = 0;
		break;
	case DOUBLE_PUSH:
		//The to square is guaranteed to be empty here
		move_piece_quiet(m.from(), m.to());
...
void Position::undo(const Move m) {
	repetitionTable.removePosition(hash);
	hash ^= zobrist::sideKey;
	moveRule = history[game_ply].moveRule;
...
public:
	explicit MoveList(Position& p) {
		last = (p.turn() == WHITE) ? p.generate_legals<WHITE>(list) : p.generate_legals<BLACK>(list);
	}
