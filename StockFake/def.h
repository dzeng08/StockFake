...
#define hashFlagExact 3


#define MAX_PLY 64
#define MATE 30000

#define noHashEntry 32767
#define VALUE_NONE 32002

const int polyglotPiece[15] = { 1, 3, 5, 7, 9, 11, -1, -1, 0, 2, 4, 6, 8, 10, -1 };
const std::string piecePromotion[5] = { "", "N", "B", "R", "Q" };

const int materialCount[15] = { 1, 3, 3, 5, 9, 0, 0, 0, 1, 3, 3, 5, 9, 0 };
...
inline int manhattanCenterDistance(int sq) {
	int file, rank;
	file = sq & 7;
	rank = sq >> 3;
	file ^= (file - 4) >> 8;
...
inline int manhattanDistance(int sq1, int sq2) {
	return abs((sq1 & 7) - (sq2 & 7)) + abs((sq1 >> 3) - (sq2 >> 3));
}

inline int getLS1BIndex(U64 number) {
