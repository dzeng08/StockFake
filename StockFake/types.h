...
public:
	...
	inline int promotion() const {
		return (move >> 12) & 0b0011;
	}

	inline bool is_enPassant() const {
		return (move >> 12) == EN_PASSANT;
	}
	...
	inline std::string toString() { return (squareToCoordinates[from()] + squareToCoordinates[to()]) + (is_promotion() ? piecePromotion[promotion() + 1] : ""); }
};
